...
def evaluate_policy(
    ...
    """
    everything the same as stable_baselines3.common.evaluation.evaluate_policy,
    except that the predict is called additionally with task_ids
    ...
    #TODO: this is not applicable when we have multiple instances of the same envs
    try:
        task_ids = env.task_ids
    except:
        # task_ids = np.array(list(range(env.num_tasks))).repeat(env.num_envs/env.num_tasks)
        task_ids = np.array(list(range(env.num_envs)))
